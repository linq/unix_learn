...
void clear_all_screen() {
  clear();
  mvprintw(2, Q_LINE, "%", "CD Database Application");
  if (current_cd[0]) {
...
void add_record() {
  ...
  mvprintw(screenrow, screencol, "About to add this new entry:");
  ...
  if (get_confirm()) {
    ...
    strcpy(current_cdt, catalog_number);
  }
}
