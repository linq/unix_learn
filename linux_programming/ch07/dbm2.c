...
{
  struct test_data items_to_store[ITEM_USERD];
  struct test_data item_received;
  ...
  strcpy(items_to_store[2].misc_chars, "Thrid");
  items_to_store[2].any_integer = 3;
  strcpy(items_to_store[2].more_chars, "baz");

  for (i = 0; i < ITEM_USERD; i++) {
  ...
  for (key_datum = dbm_firstkey(dbm_ptr); key_datum.dptr; key_datum = dbm_nextkey(dbm_ptr)) {
    ...
    if (data_datum.dptr) {
      ...
      memcpy(&item_received, data_datum.dptr, data_datum.dsize);
      printf("Received item - %s %d %s\n",
          item_received.misc_chars,
          item_received.any_integer,
          item_received.more_chars);
    } else {
